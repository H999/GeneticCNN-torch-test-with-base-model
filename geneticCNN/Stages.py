...
class Stages(torch.nn.Module):
    ...
    def get_gen(self, mode='tuple'):
        if mode == 'tuple':
            return tuple(s.binary_code for s in self.stages)
        if mode == 'model':
            return {'S_{}'.format(i + 1): '-'.join(s.separated_connections) for i, s in enumerate(self.stages)}
        raise ValueError("Only 2 mode \'tuple\' or \'model\'")
